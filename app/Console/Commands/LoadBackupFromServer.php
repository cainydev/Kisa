...
class LoadBackupFromServer extends Command
{
     ...
     * @var string
     */
    protected $signature = 'backup:latest {--no-migrate : Restore the dump as-is, without migrating afterwards}';
    ...
    /**
     * Bring the restored dump up to the current schema.
     *
     * A dump is a point-in-time snapshot: its `migrations` table reflects what
     * prod had run when the backup was taken, so anything committed since is
     * genuinely pending and has to run for the local database to match the
     * checkout.
     */
    private function migrate(): bool
    {
        ...
        if ($this->call('migrate', ['--force' => true]) !== static::SUCCESS) {
            $this->error('Migrations failed. The data is restored; fix the schema and re-run `php artisan migrate`.');
